...
Array.prototype.selfFilter = function(fn, context){
  ...
  for(let i = 0, len = sourceArr.length; i < len; i++) {
    ...
    // fn 返回值为 true时
    fn.call(context, sourceArr[i], i, sourceArr) && arr.push(sourceArr[i])
  }
  return arr
}
...
reduce
```js
Array.prototype.selfFilter1 = function(fn, context){
  // 不是函数时，报错
  if(!fn || typeof fn !== 'function') {
    throw  TypeError(`${fn} is not a function`)
  }
  ...
  arr = sourceArr.reduce((prev, next, index) => {
    fn.call(context, next, index, sourceArr) && prev.push(sourceArr[index])
    return prev
  },[])
  return arr
}
