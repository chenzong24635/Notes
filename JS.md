...
## <a name="并发（concurrency）和并行（parallelism）区别">并发（concurrency）和并行（parallelism）区别</a>

并发和并行都可以是很多个线程，
就看这些线程能不能同时被（多个）cpu执行，如果可以就说明是并行，而并发是多个线程被（一个）cpu 轮流切换着执行。

例：
并发：做 A 事件中途停下来去做 B 事件，做完B事件后继续 A 事件（走一会后，停下来吃东西，吃完了再走）
并行：做 A 事件同时做 B 事件（边走边吃）
